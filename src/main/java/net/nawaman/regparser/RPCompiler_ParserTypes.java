...
public class RPCompiler_ParserTypes {
		...
		@Override public Object  doCompile(ParseResult pThisResult, int pEntryIndex, String pParam, CompilationContext pContext,
			...
			if(!Name.equals(pThisResult.getTypeNameOfSubOf(pEntryIndex)))
				throw new RPCompilationException("Mal-formed RegParser Escape near \""
						+ pThisResult.getOriginalString().substring(pThisResult.getStartPosition()) + "\".");
	...
	static public class RPTType extends PType {
		...
		@Override public Object  doCompile(ParseResult pThisResult, int pEntryIndex, String pParam, CompilationContext pContext,
			...
			String T = pThisResult.getLastStrMatchByName("#AsText");
			String O = pThisResult.getLastStrMatchByName("#TypeOption");
			String V = pThisResult.getLastStrMatchByName("#Validate");
			String C = pThisResult.getLastStrMatchByName("#Collective");
		...
		@Override public Object  doCompile(ParseResult pThisResult, int pEntryIndex, String pParam, CompilationContext pContext,
			...
			switch(Q.charAt(0)) {
				...
				case '{': {
					...
					String BS = pThisResult.getLastStrMatchByName("#BothBound");
					int B = -1;
					if(BS == null) {
						String US = pThisResult.getLastStrMatchByName("#UpperBound");
						String LS = pThisResult.getLastStrMatchByName("#LowerBound");
						...
						if((U != -1) && (U < L))
							throw new RPCompilationException("Upper bound must not be lower than its lower bound "
									+ "near \"" + pThisResult.getOriginalString().substring(pThisResult.getStartPosition())
									+ "\".");
						if(U != L) {
							if((L == 0) && (L == 1)) {
								if(G           == null)                             return Quantifier.ZeroOrOne;
	...
	static public class RPTRange extends PType {
		...
		@Override public Object doCompile(ParseResult pThisResult, int pEntryIndex, String pParam, CompilationContext pContext,
			...
			String S  = pThisResult.getLastStrMatchByName("#Start");
			char   SC = S.charAt(0);
			if(S.length() > 1) {
			...
			String E = pThisResult.getLastStrMatchByName("#End");
	...
	static public class RPTRegParserItem extends PType {
		...
		@Override public Object doCompile(ParseResult pThisResult, int pEntryIndex, String pParam, CompilationContext pContext,
			...
			if("#Group".equals(PName)) {
				...
				String M  = pThisResult.getLastStrMatchByName("#Multiple");     M = (M == null)?"":M;
	...
	static public class RPTRegParser extends PType {
		...
		@Override public Object  doCompile(ParseResult pThisResult, int pEntryIndex, String pParam, CompilationContext pContext,
			...
			if((pThisResult.getResultEntryAt(pEntryIndex) == null) ||
				...
				throw new RPCompilationException("Mal-formed RegParser Type near \""
							+ pThisResult.getOriginalString().substring(pThisResult.getStartPositionOf(0)) + "\".");
			}
			pThisResult = pThisResult.getResultEntryAt(pEntryIndex).getSubResult();
			...
			for(int i = 0; i < Count; i++) {
				...
				if("#Error[]".equals(PName)) {
					...
					throw new RPCompilationException("Mal-formed RegParser Type near \""
							+ pThisResult.getOriginalString().substring(pThisResult.getStartPositionOf(i)) + "\".");
				}
