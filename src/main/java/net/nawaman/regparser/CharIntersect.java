...
public class CharIntersect extends CharChecker {
		...
		for(int i = 0; i < pCharCheckers.length; i++) {
			...
			if(C != null) {
				if(C instanceof CharIntersect) {
					...
					for(int c = 0; c < CI.CharCheckers.length; c++ ) CCs.add(CI.CharCheckers[c]);
	...
	@Override public boolean inSet(char c) {
		for(CharChecker CC : this.CharCheckers) if(!CC.inSet(c)) return false;
		return true;
	}
	...
	@Override public boolean equals(Object O) {
		...
		if(!(O instanceof CharIntersect)) return false;
