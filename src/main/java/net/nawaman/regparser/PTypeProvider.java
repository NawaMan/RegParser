...
public interface PTypeProvider extends Serializable {
	...
	static public class Simple implements PTypeProvider {
		...
		static public boolean exclusivelyInclude(PTypeProvider pTP, PType pPT) {
			if((pPT == null) || (pPT.TProvider != null)) return false;
			pPT.TProvider = pTP;
			return true;
		}
		...
		protected boolean addRPType(PType pRPT) {
			if(pRPT == null)                                                         return false;
			if((this.RPTypes != null) && (this.RPTypes.containsKey(pRPT.getName()))) return false;
			
			if(this.RPTypes == null) this.RPTypes = new Hashtable<String, PType>();
		...
		/** Returns the names of all types in this provider */
		public Set<String> getAllErrorMessageNames() {
			if(this.ErrMsgs == null) return null;
		...
		protected boolean addErrorMessage(String pErrName, String pErrMsg) {
			...
			if((this.ErrMsgs != null) && (this.ErrMsgs.containsKey(pErrName))) return false;
			
			if(this.ErrMsgs == null) this.ErrMsgs = new Hashtable<String, String>();
		...
		static public int loadTypeProviderFromStream(InputStream pIS, PTypeProvider pProvider, boolean isToReplace)
			...
			for(int i = Ts.length; --i >= 0; ) {
				...
				if(!isToReplace && ((Simple)pProvider).RPTypes.containsKey(N))
					((Simple)pProvider).RPTypes.put(N, T); t++;
			}
			return t;
		}
		/** Load types a stream */
		static public PType[] loadTypesFromStream(InputStream pIS) throws IOException {
			...
			for(Serializable S : Ss) {
				if(!(S instanceof PType) && !(S instanceof PTypeProvider)) continue;
				if(S instanceof PType) Types.add((PType)S);
				else                    Providers.add((PTypeProvider)S);
			}
	...
	static public class Extensible extends Simple {
		...
		@Override public boolean addRPType(PType pRPT) {
			return super.addRPType(pRPT);
		}
		@Override public boolean addErrorMessage(String pErrName, String pErrMsg) {
		...
		public boolean addType(PType pRPT) {
			if((pRPT != null) && (pRPT.TProvider == null)) pRPT.TProvider = this;
			return super.addRPType(pRPT);
		}		
		...
		public boolean addType(String pName, Checker pChecker, RPVerifier pVertifier, RPCompiler pCompiler) {
			...
			else pChecker = RegParser.newRegParser(this, pChecker);
	...
	static public class Library extends Simple {
		...
		public void addProvider(PTypeProvider pProvider) {
			...
			if(this.Providers == null) this.Providers = new Vector<PTypeProvider>();
			for(int i = 0; i < this.Providers.size(); i++) {
				if(this.Providers.get(i) == pProvider) return;
			}
		...
		@Override public Set<String> getAllTypeNames() {
			HashSet<String> Names = new HashSet<String>();
			if(this.RPTypes != null) Names.addAll(this.RPTypes.keySet());
			if(this.Providers != null) {
				for(int i = 0; i < this.Providers.size(); i++) {
					Set<String> Ns = this.Providers.get(i).getAllTypeNames();
					if(Ns != null) Names.addAll(Ns);
				}
			}
			return Names;
		}
		
		@Override public PType getType(String pName) {
		...
		@Override public Set<String> getAllErrorMessageNames() {
			HashSet<String> Names = new HashSet<String>();
			if(this.ErrMsgs != null) Names.addAll(this.ErrMsgs.keySet());
			if(this.Providers != null) {
				for(int i = 0; i < this.Providers.size(); i++) {
		...
		@Override public String getErrorMessage(String pErrName) {
