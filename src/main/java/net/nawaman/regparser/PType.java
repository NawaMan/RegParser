...
abstract public class PType implements Serializable {
	...
	final public PTypeRef getTypeRef()              { return (this.DefaultRef != null)?this.DefaultRef:(this.DefaultRef = new PTypeRef.Simple(this.getName(), null)); }
	/** Return the default TypeRef of this type with the parameter */
	final public PTypeRef getTypeRef(String pParam) { return (pParam == null)?this.getTypeRef():new PTypeRef.Simple(this.getName(), pParam); }
	...
	final public boolean isText() {
		if((this.Flags & 0x80)!= 0) return ((this.Flags & 0x08) != 0);
	...
	final public boolean isCollective() {
		if((this.Flags & 0x40)!= 0) return ((this.Flags & 0x04) != 0);
	...
	final public boolean hasValidation() {
		if((this.Flags & 0x20)!= 0) return ((this.Flags & 0x02) != 0);
	...
	final public boolean isSelfContain() {
		if((this.Flags & 0x10)!= 0) return ((this.Flags & 0x01) != 0);
		String N = this.getName();
	...
	final public boolean hasFlatAlways() {
		if((this.Flags & 0x2000)!= 0) return ((this.Flags & 0x0200) != 0);
		String N = this.getName();
	...
	final public boolean isFlatSingle() {
		if((this.Flags & 0x1000)!= 0) return ((this.Flags & 0x0100) != 0);
		String N = this.getName(); 
		boolean IsSelfContain = !N.contains("+");
	...
	PTypeProvider getDefaultTypeProvider() { return this.TProvider; }
	
	/** Returns the RegParser wrapping this type */
	public RegParser getRegParser() {
		if(this.ThisRP == null) this.ThisRP = RegParser.newRegParser(this);
		return this.ThisRP;
	}
	...
	public String  getValidation_toString() { return "..."; }
	
	/** Validate the parse result */
	final public boolean validate(ParseResult pHostResult, ParseResult pThisResult, String pParam, PTypeProvider pProvider) {
		PTypeProvider TP = PTypeProvider.Library.getEither(pProvider, this.TProvider);
		return this.doValidate(pHostResult, pThisResult, pParam, TP);
	}
	
	/** Validate the parse result */
	protected boolean doValidate(ParseResult pHostResult, ParseResult pThisResult, String pParam, PTypeProvider pProvider) {
		return true;
	}
	...
	public String getCompilation_toString() { return "..."; }
	...
	final public Object compile(String pText, String pParam, CompilationContext pContext, PTypeProvider pProvider) {
		...
		if(pParam == null) RP = RegParser.newRegParser(this);
		else {
	...
	protected Object doCompile(ParseResult pThisResult, int pEntryIndex, String pParam, CompilationContext pContext,
			PTypeProvider pProvider) {
		return (pThisResult == null)?null:pThisResult.getTextOf(pEntryIndex);
	}
