...
abstract public class RPEntry implements Serializable {
	...
	static public RPEntry _new(Checker pChecker, Quantifier pQuantifier) {
		if(pQuantifier == null) return new Direct(      pChecker);
		else                    return new Direct_WithQ(pChecker, pQuantifier);
	}
	static public RPEntry _new(String pName, Checker pChecker) {
		if(pName == null) return new Direct(            pChecker);
		else              return new NamedDirect(pName, pChecker);
	}
	static public RPEntry _new(String pName, Checker pChecker, Quantifier pQuantifier) {
		if(pName == null) {
			...
			if(pQuantifier == null) return new NamedDirect(      pName, pChecker);
			else                    return new NamedDirect_WithQ(pName, pChecker, pQuantifier);
		}
	}
	static public RPEntry _new(String pName, Checker pChecker, Quantifier pQuantifier, Checker pSecondStage) {
		if(pSecondStage == null) return _new(pName, pChecker, pQuantifier);
		if(pName == null) {
			if(pQuantifier == null) return new TwoStage(new Direct(      pChecker)             , pSecondStage);
			else                    return new TwoStage(new Direct_WithQ(pChecker, pQuantifier), pSecondStage);
		} else {
			if(pQuantifier == null) return new TwoStage(new NamedDirect(      pName, pChecker)             , pSecondStage);
			else                    return new TwoStage(new NamedDirect_WithQ(pName, pChecker, pQuantifier), pSecondStage);
		}
	}
	...
	static public RPEntry _new(PTypeRef pRPTypeRef, Quantifier pQuantifier) {
		if(pQuantifier == null) return new TypeRef(      pRPTypeRef);
		else                    return new TypeRef_WithQ(pRPTypeRef, pQuantifier);
	}
	static public RPEntry _new(String pName, PTypeRef pRPTypeRef) {
		if(pName == null) return new TypeRef(            pRPTypeRef);
		else              return new NamedTypeRef(pName, pRPTypeRef);
	}
	static public RPEntry _new(String pName, PTypeRef pRPTypeRef, Quantifier pQuantifier) {
		if(pName == null) {
			...
			if(pQuantifier == null) return new NamedTypeRef(      pName, pRPTypeRef);
			else                    return new NamedTypeRef_WithQ(pName, pRPTypeRef, pQuantifier);
		}
	}
	static public RPEntry _new(String pName, PTypeRef pRPTypeRef, Quantifier pQuantifier, Checker pSecondStage) {
		if(pSecondStage == null) return _new(pName, pRPTypeRef, pQuantifier);
		if(pName == null) {
			if(pQuantifier == null) return new TwoStage(new TypeRef(      pRPTypeRef)             , pSecondStage);
			else                    return new TwoStage(new TypeRef_WithQ(pRPTypeRef, pQuantifier), pSecondStage);
		} else {
			if(pQuantifier == null) return new TwoStage(new NamedTypeRef(      pName, pRPTypeRef)             , pSecondStage);
			else                    return new TwoStage(new NamedTypeRef_WithQ(pName, pRPTypeRef, pQuantifier), pSecondStage);
		}
	}
	...
	static public RPEntry _new(PType pRPType, Quantifier pQuantifier) {
		if(pQuantifier == null) return new Typed(      pRPType);
		else                    return new Typed_WithQ(pRPType, pQuantifier);
	}
	static public RPEntry _new(String pName, PType pRPType) {
		if(pName == null) return new Typed(            pRPType);
		else              return new NamedTyped(pName, pRPType);
	}
	static public RPEntry _new(String pName, PType pRPType, Quantifier pQuantifier) {
		if(pName == null) {
			...
			if(pQuantifier == null) return new NamedTyped(      pName, pRPType);
			else                    return new NamedTyped_WithQ(pName, pRPType, pQuantifier);
		}
	}
	static public RPEntry _new(String pName, PType pRPType, Quantifier pQuantifier, RegParser pSecondStage) {
		if(pSecondStage == null) return _new(pName, pRPType, pQuantifier);
		if(pName == null) {
			if(pQuantifier == null) return new TwoStage(new Typed(      pRPType)             , pSecondStage);
			else                    return new TwoStage(new Typed_WithQ(pRPType, pQuantifier), pSecondStage);
		} else {
			if(pQuantifier == null) return new TwoStage(new NamedTyped(      pName, pRPType)             , pSecondStage);
			else                    return new TwoStage(new NamedTyped_WithQ(pName, pRPType, pQuantifier), pSecondStage);
		}
	}
	...
	public PTypeRef   getTypeRef()     { return null;           }
	public PType      getType()        { return null;           }
	public Quantifier getQuantifier()  { return Quantifier.One; }
	public RegParser  getSecondStage() { return null;           }
	...
	@Override public String toString() {
		...
		} else if(C instanceof RegParser) {
			SB.append("(");
			if(N != null) SB.append(N).append(":~");
	...
	static private class Direct extends RPEntry {
		...
		@Override public Checker getChecker() { return this.TheChecker; }
	}
	static private class Direct_WithQ extends Direct {
		...
		@Override public Quantifier getQuantifier() { return this.TheQuantifier; }
	}
	...
	static private class TypeRef extends RPEntry {
		...
		protected TypeRef(PTypeRef pRPTypeRef) {
			...
			if(this.TheTypeRef == null) throw new NullPointerException();
		}
		
		PTypeRef TheTypeRef = null;
		@Override public PTypeRef getTypeRef() { return this.TheTypeRef; }
	}
	...
	static private class Typed extends RPEntry {
		...
		protected Typed(PType pRPType) {
			...
			if(this.TheType == null) throw new NullPointerException();
		}
		
		PType TheType = null;
		@Override public PType getType() { return this.TheType; }
	}
	...
	static private class TwoStage extends RPEntry {
		...
		@Override public Checker    getChecker()     { return this.Delegate.getChecker();    }
		@Override public PTypeRef   getTypeRef()     { return this.Delegate.getTypeRef();    }
		@Override public PType      getType()        { return this.Delegate.getType();       }
		@Override public Quantifier getQuantifier()  { return this.Delegate.getQuantifier(); }
		@Override public RegParser  getSecondStage() { return this.Parser;                   }
	}
}