...
public class RegParser implements Checker, Serializable {
    ...
    protected ParseResult parseEach_P(CharSequence pText, int pOffset, int pIndex, String FN, PType FT, PTypeRef FTR,
        ...
        if (IsFPAlternative) {
            ...
            for (int c = CA.checkers.length; --c >= 0;) {
                ParseResult TryResult = IsFPAsNode ? new PRNode(pOffset, pResult)
                        : new PRTemp(pResult);
            ...
            if (IsFPNormalChecker) {
                ...
                if (IsFPRegParser) {    // RegParser
                    if (IsFPName) {
                        ParseResult TryResult = new PRNode(pOffset, pResult);
                            ...
                            if (FP instanceof CheckerAlternative) {
                                ...
                                if (FP instanceof CheckerFixeds) {
                                    ...
                                    if (pText.length() >= (pOffset + CG.neededLength())) {
                                        EndPosition = pOffset;
                                        ThisResult  = new PRNode(pOffset, pResult);
    ...
    protected ParseResult parse(CharSequence pText, int pOffset, int pIndex, int pTimes, ParseResult pResult,
        ...
        if (pResult == null)
            pResult = new PRRoot(pOffset, pText);
        if (pText == null)
            return pResult;
                    ...
                    if ((FPQ.hasNoUpperBound()) || (pTimes < FPQ.upperBound())) {    // Not yet
                        ...
                        if (IsFPAlternative) {
                            ...
                            for (int c = CA.checkers.length; --c >= 0;) {
                                ...
                                // Try the first part
                                PRTemp TryResult = new PRTemp(pResult);
