...
final public class Quantifier implements Serializable {
    ...
    public Quantifier(int pLBound, Greediness pGness) {
        this(pLBound, -1, pGness);
    }
    
    public Quantifier(int pLBound, int pUBound) {
        this(pLBound, pUBound, null);
    }
    
    public Quantifier(int pLBound, int pUBound, Greediness pGness) {
        if ((pLBound < 0) || ((pUBound >= 0) && (pLBound > pUBound)))
    ...
    public boolean isNoUpperBound() {
        return this.UBound == -1;
    }
    
    /** Returns how greedy this quali */
    public Greediness getGreediness() {
        return this.Gness;
    }
    ...
    public boolean isMaximum() {
        return this.Gness == Greediness.Maximum;
    }
    
    /** Checks if this is a minimum greediness */
    public boolean isMinimum() {
        return this.Gness == Greediness.Minimum;
    }
    
    /** Checks if this is a possessive greediness */
    public boolean isPossessive() {
        return this.Gness == Greediness.Possessive;
    }
    ...
    public String toString() {
        ...
        if (this.LBound == this.UBound)
            return "{" + this.LBound + "}" + this.Gness.getSign();
        if (this.LBound == 0)
            return "{," + this.UBound + "}" + this.Gness.getSign();
        if (this.UBound == -1)
            return "{" + this.LBound + ",}" + this.Gness.getSign();
    ...
    public boolean equals(Object O) {
        ...
        if (O == null)
            return (this.isOne_Possessive());
        
        if (!(O instanceof Quantifier))
            return false;
        Quantifier Q = (Quantifier) O;
        if (this.UBound != Q.UBound)
            return false;
        if (this.LBound != Q.LBound)
            return false;
        if (!this.Gness.equals(Q.Gness))
            return false;
        return true;
    }
