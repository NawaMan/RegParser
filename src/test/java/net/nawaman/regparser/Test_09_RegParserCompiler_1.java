...
public class Test_09_RegParserCompiler_1 {
	...
	static public void Assert(Object pValue, Object pCorrectValue) {
		if(!Util.equal(pValue, pCorrectValue)) {
			System.out.println("It's " + Util.toString(pValue) + "\n but it should be "
	...
	static public void main(String ... Args) {
		...
		} else throw new AssertionError("PR should not be null.");
		...
		RP = RegParser.newRegParser(
				new PTypeRef.Simple(RPTRegParserItem.Name), Quantifier.OneOrMore,
		...
		RP = RegParser.newRegParser(
				new PTypeRef.Simple(RPTQuantifier.Name), Quantifier.OneOrMore,
				PredefinedCharClasses.Any, Quantifier.ZeroOrMore
			);
		if(!IsQuiet) System.out.println(RP);
		...
		RP = RegParser.newRegParser(
				new PTypeRef.Simple(RPTCharSetItem.Name), Quantifier.OneOrMore,
				PredefinedCharClasses.Any, Quantifier.ZeroOrMore
			);
		if(!IsQuiet) System.out.println(RP);
		...
		if(!IsQuiet) System.out.println(((PR == null) || (PR.getEndPosition() != Text.length()))?"Not Match":"Match");
		if(!IsQuiet) System.out.println((PR == null)?"null":PR.toString());
