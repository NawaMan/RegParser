...
public class Test_11_Speed {
	...
	static public void main(String ... Args) {
		...
		if(IsQuiet) System.out.println("Speed testing ...");
		
		long S = System.currentTimeMillis();
		RegParser RP = RegParser.newRegParser("[^0-9]*");
		long E = System.currentTimeMillis();
		if(!IsQuiet) System.out.println("RegExpr   :" + (E - S));
		
		S = System.currentTimeMillis();
		Pattern   P  = Pattern.compile("[^0-9]*");
		E = System.currentTimeMillis();
		if(!IsQuiet) System.out.println("RegParser :" + (E - S));
		...
		for(int i = 0; i < 100000; i++){
			...
			switch(I) {
				...
				case 0: SB.append((char)(Rd.nextInt('Z' - 'A') + 'A')); break;
				case 1: SB.append((char)(Rd.nextInt('z' - 'a') + 'a')); break;
			}
		}
		
		String T = SB.toString();
		
		if(!IsQuiet) System.out.println(T);
		...
		if(!IsQuiet) System.out.println("RegExpr   :" + RETs.toString());
		if(!IsQuiet) System.out.println("RegParser :" + RPTs.toString());
		if(!IsQuiet) System.out.println(M.end());
		if(!IsQuiet) System.out.println(R.getEndPosition());
