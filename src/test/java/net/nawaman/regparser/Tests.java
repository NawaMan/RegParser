...
public class Tests {
	...
	static public int getCurrentRevisionNumber() {
		...
		try {
			...
			String  Line = null;
			while ((Line = InBR.readLine()) != null) { if(Line.startsWith("Revision: ")) break; }
	...
	static public void main(String ... pArgs) {
		...
		for(int i = Math.min(ExecTime.size(), RecordTimes.size());
			    i < Math.max(ExecTime.size(), RecordTimes.size());
			    i++) {
			if(     ExecTime.size() < RecordTimes.size()) ExecTime.add(RecordTimes.get(i));
			else if(ExecTime.size() > RecordTimes.size()) RecordTimes.add(ExecTime.get(i));
		}
		...
		for(int i = 0; i < History.size(); i++) {
			...
			System.out.println("Record Times #"+i+" of " +
			                   "Revision#"+Revisions.get(i)+" " +
			                   "("+Times.get(i)+" times): " +
			                   Arrays.toString(RecordTimes.toArray(new Long[0])));
		}
