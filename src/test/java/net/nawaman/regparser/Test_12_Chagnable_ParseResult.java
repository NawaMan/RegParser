...
public class Test_12_Chagnable_ParseResult {
	...
	static public void main(String ... Args) {
		...
		RegParser RP = RegParser.newRegParser(
			"var[:WhiteSpace:]+($VarName:~[a-zA-Z_][a-zA-Z0-9_]*~)" +
		...
		RegParser RPE = RegParser.newRegParser(
			"(#ValidExpr:~($Operand:~[0-9]+~)(($Operator:~[[:+:][:*:]]~)($Operand:~[0-9]+~))*~)"
		);
		...
		if(!IsQuiet) System.out.println(PR.toString());
		if(!IsQuiet) System.out.println(PR.getSubOf(3, 0).toString());
		if(!IsQuiet) System.out.println(PR.getSubOf(3, 0).getRoot().toString());
