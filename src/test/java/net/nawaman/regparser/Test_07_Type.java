...
public class Test_07_Type {
    ...
    static public void main(String... Args) {
        ...
        /* */
        if (!IsQuiet)
            System.out.println(PTypeProvider.Simple.getDefault().getType("$byte?"));
        ...
        RP = RegParser.newRegParser(new CheckerAlternative(RegParser.newRegParser("#Value_Low", RT_Int_0To4),
                RegParser.newRegParser("#Value_High", RT_Int_5To9)), Quantifier.ZeroOrMore_Maximum);
        if (!IsQuiet)
            System.out.println(RP);
        ...
        if (!IsQuiet)
            System.out.println(PR);
        if (!IsQuiet)
            System.out.println(PR.textsOf("#Value_Low").toString());
        if (!IsQuiet)
            System.out.println(PR.textsOf("#Value_High").toString());
            ...
            Checker Checker = RegParser.newRegParser(    // <([^<]|!block!)*+>
                    ...
                    new CheckerAlternative(
                            RegParser.newRegParser("#Other",
                                    RegParser.newRegParser(new CharNot(new CharSet("<>")), Quantifier.OneOrMore)),
        ...
        if (!IsQuiet)
            System.out.println(PTypeProvider.Simple.getDefault().getType("block"));
        ...
        RP = RegParser.newRegParser(RPEntry._new("#X", new PTypeRef.Simple("$byte?")), new CharSingle('x'),
        ...
        // Add the type
        PTypeProvider.Simple.getDefault().addRPType(PTBackRef.BackRef_Instance);
        RP = RegParser.newRegParser(new CharSingle('<'),
                RPEntry._new("Begin",
                        RegParser.newRegParser(new CharUnion(new CharRange('a', 'z'), new CharRange('A', 'Z')),
        ...
        if (!IsQuiet)
            System.out.println(PTypeProvider.Simple.getDefault().getType("tag"));
        RP = RegParser.newRegParser(RPEntry._new("#Block", RT_Tag));
        if (!IsQuiet)
            System.out.println(RP);
        
        PR = RP.parse("<tag attr1='value1' attr2='value2'> <p>Something <b> Some more <br /> thing </b> </p> </Tag>",
                MyRPTP);
        if (!IsQuiet)
            System.out.println(PR);
