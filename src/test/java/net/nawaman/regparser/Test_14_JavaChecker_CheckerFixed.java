...
import net.nawaman.regparser.CheckerFixeds;
...
public class Test_14_JavaChecker_CheckerFixed {
	...
	static public class TestChecker implements Checker {	
		...
		static public final TestChecker getInstance() { return TestChecker.Instance; }
		public TestChecker() {}
		...
		static public int getLengthOf(CharSequence CS, int Offset, PTypeProvider pTProvider, ParseResult pResult) {
			for(int i = Offset; i < CS.length(); i++) {
				char c = CS.charAt(i);
				if(!((c >= '0') && (c <= '9'))) return i - Offset;
			}
			return 0;
		}
		public Checker getOptimized() { return this; }
	}
	...
	static public void main(String ... Args) {
		...
		RP = RegParser.newRegParser("!javaChecker(`net.nawaman.regparser.Test_14_JavaChecker_CheckerFixed.TestChecker->getLengthOf`)!");
		PR = RP.parse("0123456fdgd");
		if(!IsQuiet) System.out.println(PR.toString());
		
		System.out.println("------------------------------------------------------------------");
		
		RP = RegParser.newRegParser("!javaChecker(`net.nawaman.regparser.Test_14_JavaChecker_CheckerFixed.TestChecker`)!");
		PR = RP.parse("012345fdgd");
		if(!IsQuiet) System.out.println(PR.toString());
		
		System.out.println("------------------------------------------------------------------");
		
		RP = RegParser.newRegParser("!javaChecker(`net.nawaman.regparser.Test_14_JavaChecker_CheckerFixed.TestChecker::Instance`)!");
		PR = RP.parse("01234fdgd");
		if(!IsQuiet) System.out.println(PR.toString());
		
		System.out.println("------------------------------------------------------------------");
		
		RP = RegParser.newRegParser("!javaChecker(`net.nawaman.regparser.Test_14_JavaChecker_CheckerFixed.TestChecker::getInstance()`)!");
		...
		PTP.addType("TestCG", new CheckerFixeds(
				new CheckerFixeds.Entry(1),
				new CheckerFixeds.Entry(4),
				new CheckerFixeds.Entry("G1", 5, PTP.getType("Number").getTypeRef()),
